...
const handleLoginRedirect = () => history.push('/login');
  // Read ?type=user or ?type=helper
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const type = params.get("type");
    if (type === "helper" || type === "user") {
      setUserType(type);
    }
  }, [location.search]);
  ...
  const handleSignup = async () => {
    if (!name || !phone || !password) {
      showToastMessageNow("All fields are required!");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showToastMessageNow("Signup Successful 🎉");

        setTimeout(() => {
          history.push("/user-workpreferences");
        }, 1500);
      } else {
        showToastMessageNow(data?.message || "Signup Failed!");
      }
    } catch (error) {
  ...
  return (
    <IonPage className="relative min-h-screen">
      ...
      <IonContent className="relative flex flex-col items-center justify-center p-6 min-h-screen">
        ...
        {/* Navbar */}
        <div className="flex items-center justify-start w-full max-w-md px-6 py-4 
          bg-white/80 backdrop-blur-sm rounded-2xl mb-6 shadow-md z-20">

          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-pink-500 p-1 rounded-full flex items-center justify-center shadow-md">
              <img src={logoImg} className="w-14 h-14 rounded-full object-cover" alt="Logo" />
            </div>

            <span className="text-2xl font-bold text-pink-600">
              {userType === "helper" ? "Helper Signup" : "User Signup"}
            </span>
          </div>
        </div>

        {/* Card */}
        <div className="bg-pink-50 rounded-3xl shadow-2xl p-8 w-full max-w-md z-20">
          <h2 className="text-3xl font-extrabold text-pink-600 mb-6 text-center">
            Create Your Account
          </h2>
...
<div className="flex flex-col mb-4">
  ...
  <input
    ...
    focus:border-pink-400 transition shadow-sm"
  />
</div>
